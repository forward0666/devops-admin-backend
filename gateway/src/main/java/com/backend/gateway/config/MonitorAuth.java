...
@Slf4j
@RefreshScope
@Component("MonitorAuth")
public class MonitorAuth extends AuthFilter<BaseAuthConfig> {

    @Value("${secure.header.monitor.secret:default-secret}")
    private String secret;

    @Value("${secure.monitor.whitelist-paths:/health}")
    private String whitelistPaths;

    @Value("${secure.monitor.authorized-methods:}")
    private String authorizedMethods;

    public MonitorAuth(@Qualifier("blockingTaskScheduler") Scheduler scheduler, CacheManager cacheManager) {

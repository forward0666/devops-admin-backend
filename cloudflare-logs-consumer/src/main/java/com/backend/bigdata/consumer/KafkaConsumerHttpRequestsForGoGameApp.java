...
@Slf4j
@Component
public class KafkaConsumerHttpRequestsForGoGameApp {
    private final CloudflareLogsMapper cloudflareLogsMapper;
    private final ExecutorService executorService;
    private final CloudflareLogsBatchConsumer batchConsumer;

    @Autowired
    public KafkaConsumerHttpRequestsForGoGameApp(
            CloudflareLogsMapper cloudflareLogsMapper,
            ExecutorService executorService,
            @Value("${cloudflare.topic:cloudflare_logs_http_requests_gogame_app}") String topic) {
        ...
        this.batchConsumer = new CloudflareLogsBatchConsumer(
                executorService,
                cloudflareLogsMapper::batchInsertCloudflareHttpLogsForGoGameApp,
                topic
        );
    }

    @KafkaListener(
            topics = "${cloudflare.topic:cloudflare_logs_http_requests_gogame_app}",

...
@Slf4j
@Component
public class KafkaConsumerHttpRequestsForDevopApp {
    private final CloudflareLogsMapper cloudflareLogsMapper;
    private final ExecutorService executorService;
    private final CloudflareLogsBatchConsumer batchConsumer;

    @Autowired
    public KafkaConsumerHttpRequestsForDevopApp(
            CloudflareLogsMapper cloudflareLogsMapper,
            ExecutorService executorService,
            @Value("${cloudflare.topic:cloudflare_logs_http_requests_devop_app}") String topic) {
        ...
        this.batchConsumer = new CloudflareLogsBatchConsumer(
                executorService,
                cloudflareLogsMapper::batchInsertCloudflareHttpLogsForDevopApp,
                topic
        );
    }

    @KafkaListener(
            topics = "${cloudflare.topic:cloudflare_logs_http_requests_devop_app}",

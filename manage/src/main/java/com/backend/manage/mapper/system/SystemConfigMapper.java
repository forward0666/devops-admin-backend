...
public interface SystemConfigMapper {
    ...
    @ResultMap("systemConfigResultMap")
    @Select("SELECT * FROM system_config WHERE is_public = true ORDER BY config_key")
    List<SystemConfigEntity> findPublicConfigs();

    @Insert("INSERT INTO system_config (config_key, config_value, config_type, description, is_public, created_at, updated_at) " +
    ...
    @Select("SELECT COUNT(*) FROM system_config WHERE config_key = #{key}")
    ...
    @ResultMap("systemConfigResultMap")
    @Select("SELECT * FROM system_config WHERE config_key LIKE CONCAT('%', #{pattern}, '%') ORDER BY config_key")

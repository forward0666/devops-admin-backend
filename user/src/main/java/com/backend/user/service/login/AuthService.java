package com.backend.user.service.login;

import com.backend.user.client.SecurityServiceClient;
import com.backend.user.dto.login.JwtGenerateRequestDto;
...
public class AuthService {
    ...
    public boolean validateToken(String token) {
        try {
            ...
            // Java 21: 使用 Map.of() 创建不可变映射
            var requestBody = Map.<String, String>of("token", cleanToken);

            // 调用安全服务验证令牌
            var response = securityServiceClient.validateToken(requestBody);
